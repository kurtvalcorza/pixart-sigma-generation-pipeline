...
**Candidate.** The static checks pass and the model-backed smoke and local pre-flight are recorded in
`MODEL_CARD.md`, but no clean supported-runtime execution of a committed notebook blob has been recorded. Promotion to
**Release-grade** requires the row in *Manual clean-runtime evidence* to be filled with a passing Kaggle or Colab run
of the exact blob under review, with no repository checkout and all three snapshots staged inside the notebook. Any
later change to the carried modules or to the notebook produces a new blob, and the registry returns to Candidate
until a clean run of that blob is recorded here.
