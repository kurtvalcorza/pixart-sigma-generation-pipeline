...
REPO_NAME = "pixart-sigma-generation-pipeline"
NOTEBOOK_NAME = "pixart_sigma_generation_colab.ipynb"
EXPECTED_PROFILE = "E2E"
EXPECTED_MODEL_ID = "PixArt-alpha/PixArt-Sigma-XL-2-512-MS"
PIPELINE_CLASS = "PixArtSigmaPipeline"
MODEL_LOAD_EXPR = (
    f"{PIPELINE_CLASS}.from_pretrained(weights_dir=WEIGHTS_DIR, base_dir=BASE_WEIGHTS_DIR, device=('cuda' if torch.cuda.is_available() else 'cpu'), use_lora=True)"
)
# The two other pinned snapshots (T5/VAE/scheduler components, CLIP scorer) are cited beside the model pin.
KNOWN_SHAS: frozenset[str] = frozenset({"2c17b4e85261cd549b4068d086b7c2ba9d468e9f", "1a25a446712ba5ee05982a381eed697ef9b435cf"})
BYOD_GATES = ("USE_BYOD",)
EXPECTED_OUTPUTS = (
    "outputs/pixart_sigma_generation_sample_captions.csv",
    "outputs/pixart_sigma_generation_frozen_grid.jpg",
    "outputs/pixart_sigma_generation_adapted_grid.jpg",
    "outputs/pixart_sigma_generation_evaluation_report.json",
    "outputs/pixart_sigma_generation_adapter",
    "outputs/pixart_sigma_generation_result.json",
)
CODE_MARKERS = (
    # Stage 3 (fixed cell): three snapshots staged and verified, the transformer loaded with the adapter attached
    "use_lora=True)",
    "fetched_pixart_sigma_t5_vae = stage_missing_base_files(BASE_WEIGHTS_DIR, allow_download=True)",
    ...
    # Stage 9: new prompt, artifact export, reload parity, provenance
...
EXTERNAL_WEIGHT_BYTES: dict[int, str] = {
    5_789_324: "60 pinned iNaturalist CC0 bird photographs (medium size) in total",
    16_570_800: "pixart adapter.safetensors from the Kaggle E2E tutorial run (448 tensors)",
}
